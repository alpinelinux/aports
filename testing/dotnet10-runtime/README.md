...
This is the .NET 10.0 package for Alpine Linux.
...
* `aspnetcore10-targeting-pack`
* `dotnet10-apphost-pack` (used by dotnet10-runtime)
* `dotnet10-hostfxr` (used by dotnet-host)
* `dotnet10-runtime`
* `dotnet10-runtime-artifacts` (aimed for internal use as bootstrap)
* `dotnet10-runtime-bootstrap` (aimed for internal use as bootstrap)
* `dotnet10-targeting-pack`
* `dotnet-host`

## How to build dotnet10 on Alpine
...
In summary, dotnet10 is built using three different aports.

* `community/dotnet10-stage0`
Builds minimum components for full build of dotnet10, and packages these in an initial 
`dotnet10-stage0-bootstrap` package that `dotnet10-runtime` pulls.
* `community/dotnet10-runtime`
Builds full and packages dotnet10 fully using either stage0 or previoulsy built
dotnet10 build.
* `community/dotnet10-sdk`
As abuild does not allow different versions for subpackages, a different aport
is required to package sdk bits from dotnet10-runtime. dotnet10-runtime only
builds 10.0.1xx feature branch of SDK. Thus, when a new feature branch of sdk is
released, the updated components are to be built on dotnet10-sdk rather than
simply repackaging dotnet10-runtime artifacts.
...
3. Crossbuild `dotnet10-stage0` via:
```
...
In addition, the package is named `dotnet10` as opposed to `dotnet-10.0`
...
6. Commit the changes to the git repo in a git branch
    ...
    - `git checkout -b dotnet10/<name>`
    - `git add` any new patches
    - `git remove` any now-unnecessary patches
    - `git commit -m 'community/dotnet10-runtime: descriptive description'`
    - `git push`
...
7. Commit the changes to the git repo in a git branch.
    ...
    - `git add` any new patches
    - `git remove` any now-unnecessary patches
    - `git commit -m 'community/dotnet10-runtime: upgrade to <new-version>`
    - `git push`
